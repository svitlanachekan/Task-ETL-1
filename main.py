...
print(df["email"].str.split("@").str[-1].value_counts().head(5)) # проходимось з початку до кінця ; коли звертаємось до спліта то отримаємо рядочек, хочемо по стандарту до списочку. Коли пишемо 0, 1 дає саме ячейку за цим індексом, але нам потрібно; звертаюсь до рядка і в кінці -1; застосовуємо Вивів: що зустрічається
print(df["city"].value_counts().head(3)) # value_counts - назва міст

# кількість людей по містах знаходимо по попередній фільтрації, тому що скількі міст, і там стільки живут і людей
# agg_by_city = df.groupby().agg(
#     people_count = ("city", "size"),
#     avg_people = ("first_name", "mean")
# )#.sort_values(people_count).head(10) # групуємо дані за кількістю:  опрацювати скільки людей, звертаємось до df - передаю до agg кількість людей (3 людини до 1 міста) та беремо окремо імена
# print(agg_by_city) # обʼєднуємо міста та людей

df["domain"] = df["email"].str.split("@").str[-1] # створюємо стовпчик доменів, 

agg_by_city = df.groupby("city").agg(
    people_count = ("first_name", "count"), # змінна, яка буде показувати імʼя та команда - підрахувати та обʼєднуємо назву 1)стовпчика з 2)командую, яку потрібно виконати
    uniq_dom = ("domain", "nunique") # виводимо кількість n унікальних
).sort_values("people_count", ascending=False).head(10) # ascending - від більшого до меншого
